...
# 获取所有品类的内容和属性
def getdata(path, targetpath, errorpath):
        ...
        for ca3c in range(2, colN + 1):  # 从第二列开始循环
            ...
            for ca3r in range(3, rowN + 1):  # 从第三行开始循环
                ...
                if w is not None:
                    ...
                    dataR = w
                    # dataR = re.sub("[/,/]", "", w)
                    # dataR = str(dataR).replace("/", ".")
                    # 查找该品类属性文件对应路径
                    path = getpropertiespath(dataR, targetpath)
                    if len(dataR) >= 128:
                        print("错误：名字长度大于128，三级品类名【" + dataR + "】", file=errorpath)
                    if path is None:
                        print("错误：查找不到名为【" + dataR + ".xlsx】文件,将不会生成该品类的属性SQL", file=errorpath)
                        listMiss.append(dataR)
                        listprop = None
                    else:
                        # 读取该文件名的属性，并返回
                        listprop = getproperties(w, path, errorpath)
                        if listprop == None:
                            listprop = None
...
# 获取三级品类的属性值
def getproperties(categoryName, path, errorpath):
    wb = load_workbook(path)
    sheet_names = wb.get_sheet_names()
    ...
    for name in data_dic.get(2):
        ...
        if name == None:
            ...
            print('提醒：属性名出现错误，请检查文件【{0}】第【{1}】列,可以删除该列 '\
                  .format(categoryName, indexErr), file=errorpath)
            '''
            continue
    ...
    if len(listName) == len(listValue):
        ...
        print('错误：属性值与属性不匹配' + ws, file=errorpath)
    return listName


# 生成SQL语句
def getsql(data, f, errorpath):
    ...
    if data == None:
        return print('错误：获取不到生成SQL的数据', file=errorpath)
    propid = 0
    for categoryOne in data:
        ...
        if categoryOneCode < 10:
            ...
            return print('错误：第一品类数量大于99个，请重新计算')
        ...
        for categoryTwo in categoryTwoAll:
            ...
            if categoryTwoCode < 10:
                ...
                return print('错误：第二品类数量大于999个，请重新计算')
            ...
            for categoryThree in categoryThreeAll:
                ...
                if categoryThreeCode < 10:
                    ...
                    return print('错误：第三品类数量大于999个，请重新计算')
                ...
                if properties is not None:
                    for prop in properties:
                        ...
                        if len(propvalues) == 0:
                            print("提醒：三级品类【{0}】的属性【{1}】没有属性值，属性值SQL将不会生成，请修改属性文件或者至PMS系统添加"
                                  .format(category_name3, category_property_name), file=errorpath)
                        if propvalues is not None:
                            for propvalue in propvalues:
                                ...
                                if str(propvalue).find("'") > 0:
                                    ...
                                    print("提醒：表【pms_product_category_property_value】内【category_property_id】为【{0}】"
                                          "属性值【{1}】内含有错误标点【'】，已替换成【.】，"
                                          "如需更改请至数据库变更".format(category_property_id, propvalue), file=errorpath)
                            ...
                            print("提醒：三级品类【{0}】的属性【{1}】没有属性值，属性值SQL将不会生成，请修改属性文件或者至PMS系统添加"
                                  .format(category_name3, category_property_name), file=errorpath)
    print('运行完成，生成SQL文件路径为【{0}】，生成log文件路径为【{1}】，请确认log文件没有问题'.format(f.name, errorpath.name))
...
#  输出SQL的文件路径
f = open("F:/sql.sql", "w+", encoding='utf-8')
#  输出ERROR的文件路径
error = open("F:/error_log.log", "w+", encoding='utf-8')
#  输出SQL到文件
getsql(getdata(raadpath, targetPath, error), f, error)
'''
