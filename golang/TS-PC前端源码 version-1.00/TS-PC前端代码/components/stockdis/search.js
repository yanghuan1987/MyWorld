...
class GoodsSearch extends React.Component {
	...
	changePosition(){		
		...
		let d=this.state.goodsCode.split("-");
		if(d.length!=3){
			const modal = Modal.error({
	      content: '订单包裹编码格式不正确',
	    });
		...
		this.props.onChangePosition({
			sid: this.state.shelfNumber,
			number: this.state.goodsCode
		})
	}
		    ...
		    <Row gutter={24}>
				...
				<Row gutter={24}>
					<Col lg={8} md={8} sm={8} xs={24}>
						...
						<Input id="shopCodeNumber" placeholder="扫描或输入包裹编码" onChange={(e)=>this.inputChange('goodsCode',e)}/>
					</Col>
					<Col lg={8} md={8} sm={8} xs={24}>
						<span>货架ID：</span>
						<Input id="shopCodeId" placeholder="扫描或输入货架ID" onChange={(e)=>this.inputChange('shelfNumber',e)}/>
					</Col>
					<Col lg={8} md={8} sm={8} xs={24}>						
					</Col>
				...
				<Row gutter={24}>
					<Col lg={24} md={24} sm={24} xs={24}>
						...
						<Button type="primary" onClick={()=>{this.changePosition()}}>确认移动</Button>
						</div>
					</Col>
