 ...
 *
 */
public class CProducerToWMS implements Serializable{
    ...
    public Boolean doSend(PmsCommodityMsgTemp pmsCommodityMsgTemp) {
        ...
        Session session;
        //消息的目的地WMS
        Destination destinationToWMS;
        //消息生产者
        MessageProducer messageProducerToWMS;
        //实例化连接工厂
        connectionFactory = new ActiveMQConnectionFactory(CProducerToWMS.USERNAME, CProducerToWMS.PASSWORD, CProducerToWMS.BROKEURL);
        try {
            //通过连接工厂获取连接
            ...
            destinationToWMS = session.createQueue("pms_commodity_wms");
            
            //创建消息生产者
            messageProducerToWMS = session.createProducer(destinationToWMS);
            //发送消息
            sendMessage(session, messageProducerToWMS,pmsCommodityMsgTemp);
