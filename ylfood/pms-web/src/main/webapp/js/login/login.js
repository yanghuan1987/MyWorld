...
app.controller('loginCtrl', function($scope,$http) {
	...
	$scope.imageUrl = $("#webUrl").val() + "/pms/validatecode/creatvalidatecode";
	//登录
	...
	$scope.login = function() {
		...
		var response = $http.post("./pms/validatecode/checkvalidatecode?code="+$scope.confromtext);
		response.success(function(data, status, headers, config) {
		    ...
		    var response = $http.post("./pms/login/login", $scope.employeeDTO);
			response.success(function(data, status, headers, config) {
				if(data != null && data != 'success'){
					var errorcode = data;
					if(errorcode == 1){
					...
					window.location = "./pms/login/main";
				}
			});
    ...
    $scope.changeImage = function() {
    	$scope.imageUrl = $("#webUrl").val() + 
    		"/pms/validatecode/creatvalidatecode?"+Math.random();
    };
