angular
		...
		.controller(
				...
				function($scope, $rootScope, $http) {
					...
					// 产品的规格基础数据
					$http.get("getAllBasicMeasureUnit").success(function(data) {
												...
												.success(
														function(data) {
															...
															for (x in $scope.BasicWeightUnit) {
																$rootScope.weightU
																		.push($scope.BasicWeightUnit[x].optionValue);
																$rootScope.weightN
																		.push($scope.BasicWeightUnit[x].optionName);
															}
															// 过滤重量单位意外的单位
															for (i in BasicWeightUnit) {
																var match = BasicPurchaseUnit
																		.filter(function(
																				e) {
															...
															$rootScope.BasicPurchaseUnitCopy = angular
																	.copy($rootScope.BasicPurchaseUnit);
														});
									});
					// 获取所有的包装单位列表，例如瓶、袋、箱、框等等。
					$http.get("getProductStatus").success(function(data) {
						$scope.productStatus = data;
					});
					// 获取温区
					$http.get("getProductTemperatureZone").success(
							function(data) {
								$scope.severProductTemperatureZone = data;
							});

					// 获取税率
					$http.get("getTaxRate").success(function(data) {
						$rootScope.severTaxRate = data;
					});

					// 获取时间单位
					$http.get("getDateUnit").success(function(data) {
					...
					$scope.updateTwo = function(cat1name, updatecattwo) {
						...
						$http.post(
								...
								function(data) {
									...
									$scope.selectByPage($scope.pageNum,
											$scope.pageSize,
											updatecattwo.categoryCode, null,
											null, null,
											updatecattwo.categoryTypeFlag);
									$scope.initd = 0;
								});
						// 显示和收起菜单
					...
					$scope.selectByPage = function(pageNum, pageSize,
							categoryCode, productCode, productProperty,
							saleFlag, compositeFlag, productGs1Code,
							productName, productStatus,
							productTemperatureZoneCode) {
						if ($rootScope.SearchType == 1) {
							var temp = {
								...
								productTemperatureZoneCode : productTemperatureZoneCode
							// 产品温区
							}
						} else {
							var temp = {
					...
					$scope.showThree = function(cat3) {
						...
						$scope.compositeFlag = cat3.categoryTypeFlag;// 组合flg
						// 商品编辑页面状态
						...
						onChange : function() {
							...
							if ($rootScope.SearchType == 1) {
								$scope.selectByPage($scope.pagination.pageNum,
										...
										$scope.saleFlags,
										$scope.compositeFlags,
										$scope.productGs1Codes,
										$scope.productNames,
								...
								$scope.selectByPage($scope.pagination.pageNum,
										...
										$scope.saleFlag, $scope.compositeFlag);
							}
						}
					};
					...
					$scope.returnPage = function(parm){
						...
						if(parm == 'edit'){
							...
							$scope.editProduct = false;
							$scope.editCProduct = false;// 组合新增显示
					...
					$scope.searchByProductCode = function(productCode) {
						if (productCode == 'detail') {
							...
							$scope.selectByPage($scope.pageNum,
									$scope.pageSize, null, $scope.productCodes,
									null, $scope.saleFlags,
									$scope.compositeFlags,
									$scope.productGs1Codes,
									$scope.productNames, $scope.productStatuss,
									$scope.productTemperatureZoneCodes);
							$scope.initd = 0;
						} else {
							$rootScope.SearchType = 0;// 详细检索页面
							...
							$scope.selectByPage($scope.pagination.pageNum,
									$scope.pagination.pageSize,
									$scope.categoryCode, $scope.productCode,
									$scope.productProperty, null,
									$scope.compositeFlag);
						}
					};
					...
					$scope.searchByIsSalingFlag = function(saleFlag) {
						...
						$scope.selectByPage($scope.pagination.pageNum,
								$scope.pagination.pageSize,
								$scope.categoryCode, null, null, saleFlag,
								$scope.compositeFlag);
					}
					...
					$scope.onAddCustomerPropertyClick = function(property,
						...
						if (property != undefined) {
							$scope.customerProductProperty = {
								productPropertyName : property.productPropertyName,
								productPropertyValue : property.productPropertyValue
							};
					...
					// 新增自定义属性触发的事件
					$scope.onAddProductPackUnitClick = function(productBuyUnit,
							packUnit, index) {
						// 新增时 先选择产品基本单位
					...
					$scope.checkShelfLifeFromat = false;
					// 保质期check
					$scope.checkshelfLife = function(parm) {
						...
						if (undefined != parm && "" != parm) {
							if (null == parm.toString().match(checkNumber)) {
								...
								$scope.ShelfLifeUnitIsCheck = true;
								$scope.checkShelfLifeFromat = false;
							}
						} else {
							$scope.ShelfLifeUnitIsCheck = false;
					...
					$scope.onAddProductPackUnitItemClick = function(
						...
						if ($scope.packUnitEdit) {
							...
							if ($rootScope.productPackUnit == undefined
									|| $rootScope.productPackUnit == null) {
					...
					$scope.rowClick = function(index,edited) {
						...
						$http(
								{
									method : "GET",
									url : "getProductPackUnit?productCode="
											+ product.productCode
						...
						$http(
								{
									method : "GET",
									url : "getProductPicture?productCode="
											+ product.productCode
								})
								.success(
										function(data) {
											...
											} else if (product.compositeFlag == 1) {
												$rootScope.showPage = 7;
												if($rootScope.SourcePage != 1){
													$rootScope.SourcePage = 2;//跳转前页面
												}
												// 组合产品的显示
		...
		.controller(
				...
				function($scope, $rootScope, $http) {
					...
					$scope.productSpecificationUnitSecondChange = function(
							value) {
						$rootScope.BasicPurchaseUnit = angular
								.copy($rootScope.BasicPurchaseUnitCopy);
					...
					$scope.productImageDetail = function(data) {
						...
						if ($scope.addedProduct.saleFlag == undefined
								|| $scope.addedProduct.saleFlag == null) {
						...
						// 保质期
						if (undefined != $scope.addedProduct.shelfLife
								&& "" != $scope.addedProduct.shelfLife) {
						...
						// 页面新增数据上传
						$http
								.post("addProduct", $scope.addedProduct)
								.success(
										function(data) {
											// 新增完成。FLG重置
						...
						// 新增完成后进入查看界面显示该产品
						$rootScope.showPage = 5;// 普通产品编辑页面总体不显示
					}
					...
					$scope.checkproductGs1Code = function() {
						...
						if ($scope.addedProduct.productGs1Code != undefined
							...
							if ($scope.addedProduct.productGs1Code.length > 0) {
								// 查询gs1编码格式正确性
								var response = $http
										.post("/pms-web/pms/validatecode/checkvalidateGS1Code?code="
												+ $scope.addedProduct.productGs1Code);
								response.success(function(data, status,
										headers, config) {
									$scope.$parent.serverGs1Code = data;
								});
								// 查询数据库gs1编码格是否存在
								var res = $http.post("checkGS1IsExist",
										$scope.addedProduct);
		...
		.controller(
				...
				function($scope, $rootScope, $http) {
					...
					$scope.productSpecificationUnitSecondChange = function(
							value) {
						var match = $rootScope.specificationSecondValues
								.filter(function(e) {
					...
					$scope.onEditClick = function() {
						...
						$scope.$parent.Gs1Exist = true;
						$rootScope.SourcePageEdit = 5;//跳转前页面
					...
					$scope.bzcpImage = function(data) {
						...
						// 保质期
						if (undefined != $scope.product.shelfLife
								&& "" != $scope.product.shelfLife) {
							// 时间Value赋值
						...
						// 更新页面数据
						$http
								.post("updateProduct", $scope.product)
								.success(
										function(data) {
											...
											if($rootScope.SourcePage == 1){
												$scope.searchByProductCode('detail');
											}else if($rootScope.SourcePage == 2){
												$scope.selectByPage(
		...
		.controller(
				'compositeProductDetailController',// 组合产品编辑
				function($scope, $rootScope, $http) {
					...
					$scope.checkproductGs1Code = function() {
						...
						if ($scope.compositeProduct.product.productGs1Code != undefined
							...
							if ($scope.compositeProduct.product.productGs1Code.length > 0) {
								...
								var response = $http
										.post("/pms-web/pms/validatecode/checkvalidateGS1Code?code="
												+ $scope.compositeProduct.product.productGs1Code);
								response.success(function(data, status,
										headers, config) {
									$scope.$parent.serverGs1Code = data;
								});
								// 查询数据库gs1编码格是否存在
								var res = $http.post("checkGS1IsExist",
										$scope.compositeProduct.product);
								res.success(function(data, status, headers,
										config) {
									$scope.$parent.Gs1Exist = data;
								});
					...
					$scope.onEditClick = function() {
						...
						$rootScope.SourcePageEdit = 7;//跳转前页面
						...
						if ($scope.productCategoryProperties != undefined)
							...
							for (var i = 0; i < $scope.productCategoryProperties.length; i++) {
								var count = 0;
								if ($scope.compositeProduct.product != undefined
										&& $scope.compositeProduct.product.productProperties != undefined)
						...
						for (var i = 0; i < $scope.compositeProduct.productItems.length; i++) {
							$scope.$parent.selectedProductItems
									.push($scope.compositeProduct.productItems[i]);
						}
					...
					$scope.onProductPropertyChange = function(index,
					...
					$scope.onProductSelectedClick = function(index) {
						...
						if (productAlreadyAddedindex == -1) {
							// 设置产品的默认数量为1
							$scope.$parent.selectedProductItems.push({
					...
					$scope.cImagef = function(data) {
						...
						for ( var image in $rootScope.thumbHeader) {
							if ($scope.compositeProduct.product.productPictures[image] == undefined
									|| $scope.product.productPictures[image] == null)
								$scope.compositeProduct.product.productPictures
										.push({
											pictureAddress : $rootScope.thumbHeader[image].pictureAddress,
						...
						// 详情页图片信息
						for ( var image in $rootScope.thumbDetail) {
							if ($scope.compositeProduct.product.productPictures[index] == undefined
									|| $scope.compositeProduct.product.productPictures[index] == null)
								$scope.compositeProduct.product.productPictures
										.push({
											pictureAddress : $rootScope.thumbDetail[image].pictureAddress,
											pictureShowPosition : 1,
						...
						// 保质期
						if (undefined != $scope.compositeProduct.product.shelfLife
								&& "" != $scope.compositeProduct.product.shelfLife) {
							// 时间Value赋值
							for (i in $rootScope.dateUnits) {
								if ($scope.compositeProduct.product.shelfLifeName == $rootScope.dateUnits[i].optionName) {
									$scope.compositeProduct.product.shelfLifeValue = $rootScope.dateUnits[i].optionValue;
								}
							}
						}
						// 货架期
					...
					// 产品属性的修改的记录
					$scope.changedProductproperties = [];
					$scope.compositeProductPropertyChange = function(name,
							value) {
						$scope.changedProductproperties
								.push({
		...
		.controller(
				...
				function($scope, $rootScope, $http) {
					...
					// 点击搜索按钮的时候对应的标准产品用于筛选
					$scope.onSearchButtonClick = function(property) {
						if ($scope.searchCategoryCode != undefined
								&& $scope.searchCategoryCode != '') {
							$http
									.post(
											"getAllProductsByCriteria",
											{
												productProperty : $scope.propertySearchCriteria,
												categoryCode : $scope.searchCategoryCode,
												compositeFlag : 0
					...
					// 判断产品的属性是否都填写
					$scope.isProductPropertyMissed = true;
					$scope.onPropertyValueChange = function(
							addedCompositeProduct) {
						var propertyLength = 0;
						// 判断图片是否上传完成
						...
						$http
								...
								.success(
										function(data) {
											...
											$rootScope.showPage = 7;// 组合产品新增页面不显示
											// 清空查询数据
											$scope.$parent.firstLevelCategories = {};
											$scope.$parent.secondLevelCategories = {};
											$scope.$parent.thirdLevelCategories = {};
