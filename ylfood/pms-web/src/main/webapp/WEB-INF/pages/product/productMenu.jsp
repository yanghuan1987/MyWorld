...
<div class="sp-menu-left" style="border-left: 5px solid #ebebeb;width: 20%;">
	...
	<div class="sp-menu" style="overflow-x: hidden;width: 40%;background-color: #4b4b4b;float: left;">
		...
		<ul class="sp-nav-menu autobreak" ng-repeat="cat1 in categoryList" style="background-color: #4b4b4b;">
				  <li class="sp-li-line" ng-click="updateOne(cat1)" id="{{cat1.categoryName}}" ng-if ="category.categoryName==cat1.categoryName" Style="cursor:pointer;height:100%;background-color:{{colorone}}" ><a style="color: #fff;" ng-cloak><img src='<%=path%>/img/pms/whiteDown.png'>&nbsp;{{cat1.categoryName}}</a></li>
				  <li class="sp-li-line" ng-click="updateOne(cat1)" id="{{cat1.categoryName}}" ng-if ="category.categoryName!=cat1.categoryName" Style="cursor:pointer;height:100%;"><a style="color: #fff;" ng-cloak><img src='<%=path%>/img/pms/whiteDown.png'>&nbsp;{{cat1.categoryName}}</a></li>
		</ul>
	</div>
		...
		<ul class="sp-nav-menu autobreak" ng-repeat="cat1 in categoryList" style="background-color: #F5F5F5;">
				  <ul class="sp-nav-menu-sec sp-nav-menu-hide" ng-show="spMenuShow == {{cat1.id}}" ng-repeat="cat2 in cat1.productCategorys">
					...
					<ul class="sp-nav-menu-third sp-nav-menu-hide" id="fruit-third" ng-show="spMenuShowT == {{cat2.id}}" ng-repeat="cat3 in cat2.productCategorys" style="background-color: #F5F5F5;">
						<li ng-click="showThree(cat3)" ng-if ="category.categoryName==cat3.categoryName" Style="cursor:pointer;height:100%;background-color:{{colorone}}"><a href="#productTable"><span ng-cloak>{{cat3.categoryName}}</span></a></li>
						<li ng-click="showThree(cat3)" ng-if ="category.categoryName!=cat3.categoryName" Style="cursor:pointer;height:100%;"><a href="#productTable"><span ng-cloak>{{cat3.categoryName}}</span></a></li>					 
					</ul>
				  </ul>
		</ul>
	</div>
